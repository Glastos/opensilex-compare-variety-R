...
server <- function(input, output, session) {
    ...
    observeEvent(input$connect, {
        ...
        aToken <- getToken("guest@opensilex.org", "guest") #getToken(input$userName, password)
        token(aToken$data)  #token <- aToken$data
        
        #Variable list acquisition and display
        scientificObjects <- getScientificObjects(token())
    ...
    observeEvent(input$updateData, {
        ...
        data1(getData(token(), variableUri = "http://www.opensilex.org/demo/id/variables/v001", objectUri = nameUriVar1)$data)
        
        numVar2 <- match(input$variety2, varietyList()$variety)
        nameUriVar2 <-  varietyList()$uri[numVar2]
        data2(getData(token(), variableUri = "http://www.opensilex.org/demo/id/variables/v001", objectUri = nameUriVar2)$data)
